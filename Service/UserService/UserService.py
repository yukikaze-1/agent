...
class UserService:
    ...
    def __init__(self):
        ...
        # 初始化数据库代理
        self.user_core_db_agent = UserCoreDBAgent()
        self.user_profile_db_agent = UserProfileDBAgent()
        self.user_settings_db_agent = UserSettingsDBAgent()
        self.user_login_logs_db_agent = UserLoginLogsDBAgent()
        ...
        if 'user_name' in update_data:
            try:
                res_core = await self.user_core_db_agent.update_users(
                    update_data=TableUsersUpdateSetSchema(user_name=update_data['user_name']),
                    update_where=TableUsersUpdateWhereSchema(user_id=user_id)
                )
                if not res_core:
