...
# Time:         2025/06/29
# Version:      0.3
# Description:  User Database Manager - 用户数据库管理器

"""
用户数据库管理器
统一管理所有用户相关的数据库操作代理，提供事务管理和连接管理功能
"""
...
class UserDatabaseManager:
    ...
    async def init_all_connections(self) -> bool:
        ...
        :return: 如果所有连接初始化成功返回True，否则返回False
        """
        try:
            # 步骤1: 使用核心代理建立主数据库连接
    ...
    async def _get_next_user_suffix(self, user_name: str) -> int:
        ...
        try:
            ...
            from Service.UserService.app.models.UserAccountDatabaseSQLParameterSchema import TableUsersQueryWhereSchema
    ...
    async def insert_user_file(self, file_data: Dict[str, Any]) -> Optional[int]:
        ...
        """
        from Service.UserService.app.models.UserAccountDatabaseSQLParameterSchema import TableUserFilesInsertSchema
        try:
            # 将字典转换为 Pydantic 模型
    ...
    async def cleanup(self):
        ...
        for agent in self.agents:
            try:
                ...
                self.logger.error(f"Error cleaning up agent {agent.__class__.__name__}: {str(e)}")
